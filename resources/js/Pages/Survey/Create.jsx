...
export default function SurveyCreate({ firstSetQuestions, secondSetQuestions, service, department, services }) {
    ...
    const [birthdate, setBirthdate] = useState('');  // for date picker
    ...
    if (birthdate) {
        ...
        // Only update if age is different from current value
        if (data.respondent.age !== age) {
            setData('respondent', {
                ...data.respondent,
                age: age,
            });
...
}, [birthdate, setData]); // data.respondent is NOT a dependency
                ...
                <div className="relative z-10 w-full px-4 sm:px-6 lg:px-8 py-6 sm:py-10 lg:py-12">
                    ...
                    <nav className="flex items-center justify-between mb-6 sm:mb-10 max-w-7xl mx-auto">
                        <div className="flex items-center space-x-3">
                            ...
                            <div>
                                <h1 className="text-xl sm:text-2xl font-bold bg-gradient-to-r from-red-700 to-red-900 bg-clip-text text-transparent">ARTA</h1>
                    ...
                    <div className="max-w-6xl mx-auto">
                        ...
                        <div className="bg-gradient-to-r from-red-600 via-red-700 to-red-800 rounded-2xl p-6 sm:p-8 text-white shadow-2xl mb-8">
                            <div className="flex flex-col sm:flex-row items-start sm:items-center justify-between mb-6">
                                ...
                                >
                                    <svg className={`w-5 h-5 mr-2 transition-transform duration-300 ${isBisaya ? 'rotate-180' : ''}`} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <form onSubmit={handleSubmit} className="space-y-8">
                            ...
                            <div className="bg-white rounded-2xl shadow-xl p-6 sm:p-8 border border-red-100">
                                <div className="flex items-center mb-6">
                                    ...
                                    <h2 className="text-xl font-bold text-gray-900">{t.respondent_info}</h2>
                                </div>

                                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                                    {/* Client Type */}
                                    <div>
                                        <label className="block text-sm font-medium text-gray-700 mb-2">{t.client_type}</label>
                                        <select
                                    ...
                                    {/* Date of Transaction */}
                                    <div>
                                        <label className="block text-sm font-medium text-gray-700 mb-2">{t.date_of_transaction}</label>
                                        <input
                                            type="date"
                                    ...
                                    <div>
                                        ...
                                        {data.respondent.age && (
                                            <p className="mt-1 text-xs text-gray-500">
                                                {t.age_calculated}: {data.respondent.age} {t.years_old}
                                            </p>
                                        )}
                                    </div>
                                    ...
                                    {/* Civil Status */}
                                    <div>
                                        <label className="block text-sm font-medium text-gray-700 mb-2">{t.civil_status}</label>
                                        <select
                                            value={data.respondent.civil_status}
                                            onChange={(e) => handleRespondentChange('civil_status', e.target.value)}
                                            className="w-full border border-gray-300 rounded-xl shadow-sm px-4 py-3 focus:ring-2 focus:ring-red-500 focus:border-red-500 transition-all duration-300"
                                            required
                                        >
                                            <option value="">{t.select_civil_status}</option>
                                    ...
                                    <div className="md:col-span-2">
                                        <label className="block text-sm font-medium text-gray-700 mb-2">{t.region_of_residence}</label>
                                        <select
                                    ...
                                    {/* Service Availed */}
                                    <div className="md:col-span-2">
                                        <label className="block text-sm font-medium text-gray-700 mb-2">{t.service_availed}</label>
                                        <select
                                ...
                                <div className="space-y-8">
                                    <div>
                                        <div className="flex items-center mb-4">
                                            <div className="h-10 w-10 rounded-full bg-purple-100 flex items-center justify-center mr-4">
                                                <svg className="h-6 w-6 text-purple-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M7 8h10M7 12h4m1 8l-4-4H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-3l-4 4z" /></svg>
                                            </div>
                                            <h3 className="text-lg font-semibold text-gray-900">{t.suggestions_title}</h3>
                                        </div>
                                        <textarea value={data.suggestions} onChange={handleSuggestionsChange} rows={4} className="w-full border border-gray-300 rounded-xl shadow-sm px-4 py-3 focus:ring-2 focus:ring-red-500 focus:border-red-500 transition-all duration-300 resize-none" placeholder={isBisaya ? "Isuwat dinhi ang imong mga sugyot..." : "Write your suggestions here..."} />
                                    </div>
                                    <div>
                                        <div className="flex items-center mb-4">
                                            <div className="h-10 w-10 rounded-full bg-yellow-100 flex items-center justify-center mr-4">
                                                <svg className="h-6 w-6 text-yellow-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M3 8l7.89 5.26a2 2 0 002.22 0L21 8M5 19h14a2 2 0 002-2V7a2 2 0 00-2-2H5a2 2 0 00-2 2v10a2 2 0 002 2z" /></svg>
                                            </div>
                                            <h3 className="text-lg font-semibold text-gray-900">{t.email_title}</h3>
                                        </div>
                                        <input type="email" value={data.email} onChange={handleEmailChange} className="w-full border border-gray-300 rounded-xl shadow-sm px-4 py-3 focus:ring-2 focus:ring-red-500 focus:border-red-500 transition-all duration-300" placeholder={isBisaya ? "example@email.com" : "example@email.com"} />
                                    </div>
                                    <div className="bg-gradient-to-r from-green-50 to-emerald-100 border border-green-200 rounded-xl p-6 text-center">
                                        <div className="flex items-center justify-center mb-3">
                                            ...
                                            <h4 className="text-xl font-bold text-green-800">{t.thank_you}</h4>
                                        </div>
                                        <p className="text-green-700">{isBisaya ? "Daghang salamat sa imong panahon ug hinanaling feedback!" : "Thank you for your time and valuable feedback!"}</p>
                                    </div>
                                </div>
                            </div>
                            ...
                            <div className="bg-gradient-to-r from-red-600 to-red-800 rounded-2xl p-8 text-white shadow-2xl">
                                <div className="text-center mb-8">
                                    ...
                                    <p className="text-red-100 text-lg">{isBisaya ? "Ang imong feedback makatabang sa pagpaayo sa serbisyo alang sa tanang Pilipino." : "Your feedback helps improve services for all Filipinos."}</p>
                                </div>
                                <div className="flex flex-col sm:flex-row items-center justify-between gap-6">
                                    <div className="text-center sm:text-left">
                                        <div className="flex items-center justify-center sm:justify-start mb-2">
                                            <svg className="h-5 w-5 text-green-300 mr-2" fill="currentColor" viewBox="0 0 20 20"><path fillRule="evenodd" d="M16.707 5.293a1 1 0 010 1.414l-8 8a1 1 0 01-1.414 0l-4-4a1 1 0 011.414-1.414L8 12.586l7.293-7.293a1 1 0 011.414 0z" clipRule="evenodd" /></svg>
                                            <span className="font-medium">{t.confidential}</span>
                                        </div>
                                        <p className="text-red-200 text-sm">{isBisaya ? "Tanang impormasyon protektado ubos sa Data Privacy Act" : "All information protected under Data Privacy Act"}</p>
                                    </div>
                                    <button type="submit" disabled={processing || !allQuestionsAnswered} className="inline-flex items-center justify-center px-8 py-4 text-lg font-bold text-red-700 bg-white hover:bg-red-50 rounded-xl shadow-lg hover:shadow-2xl transform hover:-translate-y-1 transition-all duration-300 disabled:opacity-50 disabled:cursor-not-allowed">
                                        {processing ? (
                                            <>
                                ...
                                <div className="mt-8 pt-6 border-t border-white/20 text-center">
                                    <p className="text-sm text-red-100">🚫 <span className="font-bold">NO TO PALAKASAN</span> • 🚫 <span className="font-bold">NO TO FAVORITISM</span> • 🚫 <span className="font-bold">NO TO CORRUPTION</span></p>
                                </div>
                            </div>
                        </form>
                            ...
                            <div className="flex flex-col sm:flex-row items-center justify-between mb-6">
                                <div className="flex items-center mb-4 sm:mb-0">
                                    <div className="h-8 w-8 rounded-full bg-red-600 flex items-center justify-center mr-3">
                                        <svg className="h-4 w-4 text-white" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12l2 2 4-4m6 2a9 9 0 11-18 0 9 9 0 0118 0z" /></svg>
                                    </div>
                                ...
                                <p className="text-gray-500 text-sm">This survey is conducted in compliance with Republic Act No. 11032 (Ease of Doing Business Act)</p>
                            </div>
                        </div>
                    </div>
                </div>
